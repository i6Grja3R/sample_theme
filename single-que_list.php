...
$table = function_exists('bbs_questions_table')
    ? bbs_questions_table()
    : $wpdb->prefix . 'bbs_questions';
...
WHERE is_confirmed = 1
  AND status = 'active'
  AND (
    attach1 REGEXP '\\\\.(jpg|jpeg|png)$'
    OR attach2 REGEXP '\\\\.(jpg|jpeg|png)$'
    OR attach3 REGEXP '\\\\.(jpg|jpeg|png)$'
    OR files LIKE '%.jpg%'
    OR files LIKE '%.jpeg%'
    OR files LIKE '%.png%'
  )
...
foreach ($rows as $row) {
    ...
    // 1) attach1 → attach2 → attach3 の順で画像を探す
    foreach ([$row->attach1 ?? '', $row->attach2 ?? '', $row->attach3 ?? ''] as $att) {
        if (!empty($att) && preg_match('/\.(jpg|jpeg|png)$/i', $att)) {
            $image_rel = $att;
            break;
        }
    }

    // 2) attach1〜3 が空なら files(JSON) から画像を探す
    if ($image_rel === '' && !empty($row->files)) {
        $files = json_decode($row->files, true);

        if (is_array($files)) {
            foreach ($files as $file) {
                if (is_string($file) && preg_match('/\.(jpg|jpeg|png)$/i', $file)) {
                    $image_rel = $file;
                    break;
                }
            }
    ...
    $image_url  = trailingslashit($upload_dir['baseurl']) . ltrim($image_rel, '/');
    ...
    <div class="carousel-video-area">
        <a href="' . esc_url($detail_url) . '">
            <div class="carousel-video-wrap">
                <img class="carousel-video-streaming" src="' . esc_url($image_url) . '" alt="' . esc_attr($row->title) . '">
            </div>
