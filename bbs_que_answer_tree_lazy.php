...
<style>
.bbs-thread { max-width: 980px; margin: 0 auto; }
.bbs-question { padding: 18px; background: #fff; border: 1px solid #ddd; }
.bbs-question__title { font-size: 24px; font-weight: 800; margin-bottom: 12px; }
.bbs-question__text { white-space: pre-wrap; line-height: 1.8; }
.bbs-question__media { display: flex; gap: 12px; flex-wrap: wrap; margin: 16px 0; }
.bbs-question__media img { width: 535px; max-width: 100%; height: 301px; object-fit: contain; background: #fafafa; }
.bbs-question__user { display: flex; align-items: center; gap: 10px; margin-top: 14px; }
.bbs-question__user img { width: 64px; height: 64px; object-fit: cover; border-radius: 50%; }
.bbs-archive-notice { padding: 12px; background: #f5f5f5; border: 1px solid #ddd; margin: 16px 0; }
.bbs-answer-form { margin: 24px 0; padding: 16px; background: #FCF2F4; border: 2px solid #F7859C; }
.bbs-answer-form textarea { width: 100%; min-height: 150px; padding: 10px; box-sizing: border-box; }
.bbs-answer-form input[type="text"] { width: 100%; padding: 8px; box-sizing: border-box; margin-top: 8px; }
.bbs-answer-form button { margin-top: 10px; }
.bbs-comments { margin-top: 24px; }
.bbs-comment { margin-left: calc(var(--depth-indent, 0) * 22px); padding: 12px; border-left: 3px solid #ddd; background: #fff; margin-top: 12px; }
.bbs-comment__meta { font-size: 13px; color: #666; display: flex; gap: 8px; }
.bbs-comment__name { font-weight: 700; color: #333; }
.bbs-comment__body { margin-top: 8px; line-height: 1.7; white-space: normal; word-break: break-word; }
.bbs-comment__actions { margin-top: 8px; display: flex; gap: 8px; flex-wrap: wrap; }
.bbs-children { margin-top: 8px; }
.bbs-load-more-wrap { text-align: center; margin: 20px 0; }
.bbs-loading { opacity: .65; pointer-events: none; }
</style>
...
<main class="bbs-thread" id="bbs_thread" data-thread-unique-id="<?php echo esc_attr($unique_id); ?>" data-nonce="<?php echo esc_attr($nonce); ?>" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>">
    ...
    <?php else : ?>
        <section class="bbs-answer-form" id="bbs_answer_form">
            ...
            <input type="hidden" id="bbs_reply_parent_id" value="<?php echo esc_attr((int) $thread->id); ?>">
...
(() => {
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (replyLoad) {
            ...
            try {
                const data = await postAjax('bbs_load_replies', { parent_id: parentId, cursor, limit: '10' });
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const isAnswer = parentId === '<?php echo esc_js((string) ((int) $thread->id)); ?>';
                if (isAnswer) {
                    answerList.insertAdjacentHTML('beforeend', data.html || '');
                } else {
                    const container = document.querySelector(`.bbs-children[data-parent-id="${CSS.escape(parentId)}"]`);
                ...
                textEl.value = '';
                parentEl.value = '<?php echo esc_js((string) ((int) $thread->id)); ?>';
            } catch (err) {
                alert(err.message);
            } finally {
