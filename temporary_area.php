 ...
 * ファイル操作用の安全ヘルパー
 * ==========================================================
 * Snyk の Path Traversal 警告対策:
 * - unlink / chmod / rename / copy / move_uploaded_file を直接あちこちに書かない
 * - ファイル操作前に必ず realpath() と base ディレクトリ配下チェックを通す
 * - tmp 側は /uploads/tmp/ 配下、attach 側は /uploads/attach/ 配下に限定する
 */

if (!function_exists('bbs_normalize_base_dir')) {
    function bbs_normalize_base_dir($dir)
    {
        $real = realpath($dir);
        ...
        // 比較用に WordPress 形式へ正規化し、末尾スラッシュを必ず付ける
        return rtrim(wp_normalize_path($real), '/') . '/';
    }
}

if (!function_exists('bbs_is_path_inside_base')) {
    function bbs_is_path_inside_base($path, $base_norm)
    {
        $real = realpath($path);
        ...
        // /tmp と /tmp_evil のような前方一致誤判定を防ぐため、base_norm 側は末尾 / 付きにしている
        return strpos($real_norm, $base_norm) === 0;
    }
}

if (!function_exists('bbs_safe_unlink_abs')) {
    function bbs_safe_unlink_abs($path, $base_norm)
    {
        $real = realpath($path);

        // すでに存在しないファイルは「削除済み」とみなす
        if ($real === false) {
            return true;
        }
        ...
        if (strpos($real_norm, $base_norm) !== 0) {
            return false;
        }
...
if (!function_exists('bbs_safe_chmod_abs')) {
    function bbs_safe_chmod_abs($path, $base_norm, $mode = 0644)
    {
        $real = realpath($path);
        ...
        return chmod($real, $mode);
    }
}

if (!function_exists('bbs_safe_unlink_tmp_filename')) {
    function bbs_safe_unlink_tmp_filename($filename, $tmp_dir, $base_tmp_norm)
    {
        $filename = (string) $filename;
        $filename = basename($filename);

        // submit側で作る tmp ファイル名は UUID.拡張子 のみ許可する
        if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.[a-z0-9]{1,10}$/i', $filename)) {
            return false;
        }

        $path = trailingslashit($tmp_dir) . $filename;

        return bbs_safe_unlink_abs($path, $base_tmp_norm);
    }
}

if (!function_exists('bbs_safe_move_uploaded_file_to_tmp')) {
    function bbs_safe_move_uploaded_file_to_tmp($tmp_name, $dst_path, $base_tmp_norm)
    {
        ...
        // 移動先はまだ存在しないため、親ディレクトリで検証する
        $dst_dir_real = realpath(dirname($dst_path));

        if ($dst_dir_real === false || !is_dir($dst_dir_real)) {
            return false;
        }

        $dst_dir_norm = rtrim(wp_normalize_path($dst_dir_real), '/') . '/';

        if (strpos($dst_dir_norm, $base_tmp_norm) !== 0) {
            return false;
        }

        if (!is_writable($dst_dir_real)) {
            return false;
        }

        return move_uploaded_file($tmp_name, $dst_path);
    }
}

if (!function_exists('bbs_safe_move_tmp_to_attach')) {
    function bbs_safe_move_tmp_to_attach($src_path, $dst_path, $base_tmp_norm, $base_attach_norm)
    {
        ...
        $src_real_norm = wp_normalize_path($src_real);

        // 移動元は必ず tmp 配下
        if (strpos($src_real_norm, $base_tmp_norm) !== 0) {
            return false;
        }
        ...
        // 移動先は必ず attach 配下
        if (strpos($dst_dir_norm, $base_attach_norm) !== 0) {
            return false;
        }
        ...
        // 同一ファイルシステムなら rename、環境差で失敗したら copy + unlink
        if (rename($src_real, $dst_path)) {
            return true;
        }

        if (copy($src_real, $dst_path)) {
            return unlink($src_real);
        }
...
if (!function_exists('bbs_quest_submit')) {              // 多重定義防止
    function bbs_quest_submit()                        // AJAX ハンドラ
    {
        ...
        if (isset($_FILES['attach']) && is_array($_FILES['attach']['tmp_name'])) {   // 添付が配列で届いているか
            ...
            // Snyk の Path Traversal 対策: 比較用の基底パスを正規化して保持
            $base_tmp_norm = bbs_normalize_base_dir($tmp_dir);
            if ($base_tmp_norm === false) {
                wp_send_json_error(['errors' => ['・一時ディレクトリの検証に失敗しました。']]);
            }
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($real === false || strpos(rtrim(wp_normalize_path($real), '/') . '/', $base_tmp_norm) !== 0) {
                ...
                if (!bbs_safe_move_uploaded_file_to_tmp($tmp_name, $dst, $base_tmp_norm)) {
                    wp_send_json_error(['errors' => ['・サーバーにファイルを保存できませんでした。']]);
                }

                // パーミッションを緩めに（実行権不要）
                bbs_safe_chmod_abs($dst, $base_tmp_norm, 0644);
        ...
        if ($json === false || strlen($json) > 65535) {                                 // エンコード失敗 or データ肥大
            ...
            foreach ($saved as $fn) {
                if ($fn) {
                    bbs_safe_unlink_tmp_filename($fn, $tmp_dir, $base_tmp_norm);
                }
            }
...
// ※ このファイルでは上部で既に読み込み済み。分割ファイルに戻す場合はこの行を有効化してください。
// =============================================================================
// 1) confirm 本体
// =============================================================================
if (!function_exists('bbs_quest_confirm')) {                 // 多重定義防止
    ...
    function bbs_quest_confirm()
    {
        ...
        if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{12}$/i', $draft_id)) {
        ...
        if ($mode === 'commit') {
            ...
            $base_tmp_norm    = bbs_normalize_base_dir($tmp_dir);
            $base_attach_norm = bbs_normalize_base_dir($attach_dir);

            if ($base_tmp_norm === false || $base_attach_norm === false) {             // tmp ディレクトリ自体が異常
            ...
            $ext_whitelist = [];
            foreach (bbs_allowed_upload_map() as $mime_exts) {
                foreach ((array) $mime_exts as $allowed_ext) {
                    $allowed_ext = preg_replace('/[^a-z0-9]/', '', strtolower((string) $allowed_ext));
                    if ($allowed_ext !== '') {
                        $ext_whitelist[] = $allowed_ext;
                    }
                }
            }
            $ext_whitelist = array_values(array_unique($ext_whitelist));
            ...
            foreach ((array)$files as $i => $tmp_filename) {                     // 各添付を順に処理
                ...
                $tmp_filename = basename($tmp_filename);                  // クライアント由来のパス要素を排除（basenameのみを使用）
                ...
                if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.[a-z0-9]{1,10}$/i', $tmp_filename)) {
                    $filenames[$i] = '';
                    continue;
                }

                // tmp 側絶対パス（basename 固定化）
                $src_path = trailingslashit($tmp_dir) . $tmp_filename; // クライアント由来のパス要素を排除（basenameのみを連結）
                ...
                /*
                 * realpath後の実体が tmp ディレクトリ配下にあるか確認する。
                 * シンボリックリンクなどで tmp 外へ逃げるケースもここで防ぐ。
                 */
                $src_real_norm = wp_normalize_path($src_real);
                ...
                if (!in_array($ext, $ext_whitelist, true)) {                 // 許可拡張子でなければ破棄
                    bbs_safe_unlink_abs($src_real, $base_tmp_norm);                                           // tmpファイルを削除
                    $filenames[$i] = '';
                    continue;
                }
                ...
                $final_filename = sanitize_file_name("{$prefix}_{$suffix}.{$ext}");               // 例) 550e..._attach1.jpg
                $final_filename = wp_unique_filename($attach_dir, $final_filename);
                ...
                if ($real_dst_dir === false) {
                    bbs_safe_unlink_abs($src_real, $base_tmp_norm);
                    $filenames[$i] = '';
                    continue;
                }

                $real_dst_dir_norm = rtrim(wp_normalize_path($real_dst_dir), '/') . '/';

                /*
                 * パストラバーサル対策:
                 * 移動先が必ず attach ディレクトリ配下か確認する。
                 */
                if (strpos($real_dst_dir_norm, $base_attach_norm) !== 0) {
                    bbs_safe_unlink_abs($src_real, $base_tmp_norm);
                    $filenames[$i] = '';
                    continue;
                }
                ...
                // wp_unique_filename() で重複回避済みなので、ここで自前の file_exists() 分岐は不要
                // 旧処理は重複していたため削除し、移動前のディレクトリ検証だけ残す

                // 6) attach 側ディレクトリの realpath ガード（シンボリックリンク/脱出対策）
                $real_dst_dir = realpath(dirname($dst_path));                   // まだファイルが無いので dirname() で検証
                if ($real_dst_dir === false || strpos(rtrim(wp_normalize_path($real_dst_dir), '/') . '/', $base_attach_norm) !== 0) {   // 想定外のパス（ディレクトリトラバーサル等）を拒否
                    // 想定外ディレクトリなら tmp 側を掃除してスキップ
                    bbs_safe_unlink_abs($src_real, $base_tmp_norm);                                           // tmp を削除
                    $filenames[$i] = '';
                    continue;
                }

                // 9) tmp → attach へ最終移動（rename フォールバック付き）
                if (!bbs_safe_move_tmp_to_attach($src_real, $dst_path, $base_tmp_norm, $base_attach_norm)) {                           // rename が失敗する環境（跨ぎ等）向けフォールバック
                    bbs_safe_unlink_abs($dst_path, $base_attach_norm);         // 途中生成物の掃除
                    // これまでの生成物を掃除
                    foreach ($saved_abs as $abs) {
                        bbs_safe_unlink_abs($abs, $base_attach_norm);
                    }
                    // DBロールバック
                    $wpdb->delete($table, ['id' => $insert_id], ['%d']);
                ...
                // confirm 側では最終移動先の $dst_path に対して chmod する
                bbs_safe_chmod_abs($dst_path, $base_attach_norm, 0644);                                          // 実行権不要のパーミッションに調整（web配信前提）
